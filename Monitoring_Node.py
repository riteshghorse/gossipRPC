...
class MonitoringNode:
    ...
    def updateSuspectMatrix(self, ip, fault_vector, generation):
        ...
        for k, v in fault_vector.items():
            if v == 0:
                if (
                    self.IP_to_Node_Index[k] in self.ip_generation
                    and generation == self.ip_generation[self.IP_to_Node_Index[k]]
                ):
            ...
            #     print(self.IP_to_Node_Index, k)
            #     if (self.IP_to_Node_Index[k] in self.ip_generation and k in self. and k in self.[k]):
            #         if self.[k][k]['heartBeat']['generation'] == self.ip_generation[self.IP_to_Node_Index[k]]:
            #             print('False failure detection happened for ', k)
    ...
    def doConsensus(self):
        ...
        for j in range(len(self.suspect_matrix[0])):
            ...
            for i in range(len(self.suspect_matrix)):
                if i != j and (
                    self.global_fault_vector[i] & self.global_fault_vector[j] != 1
                ):
                    if (
                        getDiffInSeconds(self.heartbeatTime[self.Index_to_IP[i]])
                        < Constants.WAIT_SECONDS_FAIL
                    ):
            ...
            if state == 1:
                ...
                # try:
                self.ip_generation[j] = self.global_state_map[self.Index_to_IP[j]][
                    self.Index_to_IP[j]
    ...
    def doConsensusCheck(self):
        ...
        for i in range(len(keyList) - 1):
            if self.global_fault_vector[self.IP_to_Node_Index[keyList[i]]] != 1:
                res = res & (
                    self.consensusMap[keyList[i]] == self.consensusMap[keyList[i + 1]]
                )
...
if __name__ == "__main__":
    ...
    server_ip = socket.gethostbyname(
        socket.gethostname()
    )  # ConfigurationManager.get_configuration().get_gossip_host()
