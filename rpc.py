...
class XMLRPCGossipManager(object):
    ...
    def start_server(gossip_node):
        ...
        if not XMLRPCGossipManager.server and not XMLRPCGossipManager.server_thread:
            XMLRPCGossipManager.server = AsyncXMLRPCServer(
                (ip, port), GossipRPCRequestHandler, allow_none=True, logRequests=False
            )
            XMLRPCGossipManager.server.register_instance(gossip_node)
            XMLRPCGossipManager.server_thread = threading.Thread(
                target=XMLRPCGossipManager.server.serve_forever
            )
